...
- 优先解决“多个基础形状 + 自动最小体积 + 用户可通过固定旋转显式切换姿态”的高频需求
...
- 不搜索尺寸
- 直接验证当前 `gridX / gridY / heightUnits` 是否可容纳当前布局
...
- 已自动增加顶部余量，以对齐 Gridfinity 标准高度单位
- 当前是固定外部尺寸，不再自动搜索更小尺寸
